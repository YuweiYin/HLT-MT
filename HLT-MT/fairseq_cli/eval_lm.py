    ...
    for sample in progress:
        ...
        for i, hypos_i in enumerate(hypos):
            ...
            if bpe_toks is not None:
                for idx in range(tgt_len - 1):
                    if tokens[idx].item() in bpe_toks:
                        skipped_toks += 1
                        pos_scores[idx + 1] += pos_scores[idx]
                        pos_scores[idx] = 0
            ...
            if cfg.eval_lm.output_word_probs or cfg.eval_lm.output_word_stats:
                ...
                for idx in range(len(tokens)):
                    w_ind = tokens[idx].item()
                    w += task.source_dictionary[w_ind]
                    if bpe_toks is not None and w_ind in bpe_toks:
                        w = w[:-bpe_len]
                        is_bpe = True
                    else:
                        word_prob.append((w, pos_scores[idx].item()))
