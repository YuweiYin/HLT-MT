...
class MultilingualDatasetManager(object):
    ...
    def add_args(parser):
        ...
        parser.add_argument(
            ...
            default=None,
            type=csv_str_list,
            help="a list of languages comma separated languages which can appear in lang-pairs; "
    ...
    def load_lang_dataset(
        ...
        if load_alignments:
            ...
            if indexed_dataset.dataset_exists(os.path.join(data_path, "{}.align.{}-{}".format(split, src, tgt)),
                                              impl=dataset_impl):
                align_path = os.path.join(
                    data_path, "{}.align.{}-{}".format(split, src, tgt)
                )
            elif indexed_dataset.dataset_exists(os.path.join(data_path, "{}.align.{}-{}".format(split, tgt, src)),
                                                impl=dataset_impl):
                align_path = os.path.join(
    ...
    def src_dataset_transform_func(self, src_lang, tgt_lang, dataset, spec=None):
        if self.args.lang_tok_replacing_bos_eos:
            ...
            # TODO: Unify with alter_dataset_langtok
            return dataset
        if spec is None:
            return dataset
    ...
    def get_train_sampling_ratios(
        ...
        sample_ratios = [sampling_func(_data_size, epoch) for _data_size in
                         data_sizes] if sampling_func is not None else None
        sample_ratios = [ratio / sum(ratios) / len(sample_ratios) for ratios in sample_ratios for ratio in
                         ratios] if sample_ratios is not None else None
    ...
    def load_sampled_multi_epoch_dataset(
        ...
        if training and split == getattr(self.args, "train_subset", None):
            ...
            return SampledMultiEpochDataset(
                ...
                # valid and test datasets will be degenerate to concat datasets:
