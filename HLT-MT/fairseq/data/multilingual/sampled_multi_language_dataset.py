...
class SampledMultiLangDataset(SampledMultiDataset):
    ...
    def ordered_indices(self):
        ...
        multi_lang_sizes = [
            multi_lang_sizes[0 if i == 0 else self.cumulated_sizes[i - 1]: self.cumulated_sizes[i]]
    ...
    def batch_by_size(
        ...
        multi_lang_sort_indices = [
            indices[0 if i == 0 else self.cumulated_sizes[i - 1]: self.cumulated_sizes[i]]
            for i in range(len(self.datasets))
        ]
        batches = []
    ...
    def collater(self, samples, **extra_args):
        ...
        if self.collate_format == "ordered_dict":
            ...
            if "alignments" in batches[0].keys() and batches[0]["alignments"] is not None:
                alignments = [b["alignments"][align_index] for b in batches for align_index in
                              range(len(b["alignments"]))]
