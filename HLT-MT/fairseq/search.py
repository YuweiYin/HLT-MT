...
class LexicallyConstrainedBeamSearch(Search):
    ...
    def init_constraints(self, batch_constraints: Optional[Tensor], beam_size: int):
        ...
        for constraint_tensor in batch_constraints:
            ...
            self.constraint_states.append([constraint_state for _ in range(beam_size)])
    ...
    def step_sentence(
        ...
        for beamno, state in enumerate(constraint_states):
            ...
            if next_tokens.numel() != 0:
                ...
                next_beams = (
                    torch.tensor(beamno, device=device).repeat(next_tokens.size(0)).long()
                )
...
class DiverseSiblingsSearch(Search):
    ...
    def step(
        ...
        s_list: List[Tensor]
        i_list: List[Tensor]
        s_list = [torch.empty(0).to(lprobs) for _ in range(beam_size)]
        i_list = [torch.LongTensor().to(device=lprobs.device) for _ in range(beam_size)]
