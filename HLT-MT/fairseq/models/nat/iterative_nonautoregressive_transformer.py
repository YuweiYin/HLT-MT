...
def gumbel_noise(input, TINY=1e-8):
    return (
        input.new_zeros(*input.size()).uniform_().add_(TINY).log_().neg_().add_(TINY).log_().neg_()
    )
        ...
        for t in range(self.train_step):
            ...
            if t < (self.train_step - 1):
                ...
                if self.dae_ratio > 0:
                    ...
                    corrupted_tokens = _sequential_poisoning(
                        tgt_tokens[corrupted],
                        len(self.tgt_dict),
                        0.33,
                        self.bos,
                        self.eos,
                    ...
                    prev_output_tokens[corrupted] = corrupted_tokens
