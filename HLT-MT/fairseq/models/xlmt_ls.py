            ...
            if 'in_proj_weight' in key:
                ...
                if i_layer % 2 == 0:
                    state_dict[key.replace('decoder.sentence_encoder.', '').replace(
                        'fc1', 'fc3').replace(f'.{i_layer}', f'.{i_layer // 2}')] = infoxlm_state_dict[key]
                else:
                    state_dict[key.replace('decoder.sentence_encoder.', '').replace(
                ...
                if i_layer % 2 == 0:
                    state_dict[key.replace('decoder.sentence_encoder.', '').replace(
                        'fc2', 'fc4').replace(f'.{i_layer}', f'.{i_layer // 2}')] = infoxlm_state_dict[key]
                else:
                    state_dict[key.replace('decoder.sentence_encoder.', '').replace(
                        f'.{i_layer}', f'.{i_layer // 2}')] = infoxlm_state_dict[key]
        ...
        if 'lm_head' not in key:
            if 'in_proj_weight' in key:
                ...
                if i_layer % 2 == 0:
                    ...
                    state_dict[
                        key.replace('decoder.sentence_encoder.', '').replace(
                            f'.{i_layer}', f'.{i_layer // 2}')] = infoxlm_state_dict[key]
            elif 'final_layer_norm' in key:
                i_layer = int(key.split('.')[3])
                if i_layer % 2 == 0:
        ...
        for key in infoxlm_state_dict.keys():
            ...
            if 'lm_head' not in key:
                if 'in_proj_weight' in key:
                    ...
                    i_layer = int(key.split('.')[3])
                    if i_layer % 2 == 0:
                        state_dict[key.replace('decoder.sentence_encoder.layers', ls_name).replace(
                            'in_proj_weight', 'q_proj.weight').replace(
                            f'.{i_layer}', f'.{ls_id}.{layers2lslayers(i_layer)}')] = q
                        state_dict[key.replace('decoder.sentence_encoder.layers', ls_name).replace(
                            'in_proj_weight', 'k_proj.weight').replace(
                            f'.{i_layer}', f'.{ls_id}.{layers2lslayers(i_layer)}')] = k
                        state_dict[key.replace('decoder.sentence_encoder.layers', ls_name).replace(
                            'in_proj_weight', 'v_proj.weight').replace(
                            f'.{i_layer}', f'.{ls_id}.{layers2lslayers(i_layer)}')] = v
                    else:
                        state_dict[key.replace('decoder.sentence_encoder.layers', ls_name).replace(
                            'in_proj_weight', 'q_proj.weight').replace(
                            f'.{i_layer}', f'.{ls_id}.{layers2lslayers(i_layer)}').replace(
                    ...
                    if i_layer % 2 == 0:
                        state_dict[key.replace('decoder.sentence_encoder.layers', ls_name).replace(
                            'in_proj_bias', 'q_proj.bias').replace(
                            f'.{i_layer}', f'.{ls_id}.{layers2lslayers(i_layer)}')] = q
                        state_dict[key.replace('decoder.sentence_encoder.layers', ls_name).replace(
                            'in_proj_bias', 'k_proj.bias').replace(
                            f'.{i_layer}', f'.{ls_id}.{layers2lslayers(i_layer)}')] = k
                        state_dict[key.replace('decoder.sentence_encoder.layers', ls_name).replace(
                            'in_proj_bias', 'v_proj.bias').replace(
                            f'.{i_layer}', f'.{ls_id}.{layers2lslayers(i_layer)}')] = v
                    else:
                        state_dict[key.replace('decoder.sentence_encoder.layers', ls_name).replace(
                            'in_proj_bias', 'q_proj.bias').replace(
                    ...
                    if i_layer % 2 == 0:
                        state_dict[
                            key.replace('decoder.sentence_encoder.layers', ls_name).replace('fc1', 'fc3').replace(
                                f'.{i_layer}', f'.{ls_id}.{layers2lslayers(i_layer)}')] = infoxlm_state_dict[key]
                    else:
                        state_dict[key.replace('decoder.sentence_encoder.layers', ls_name).replace(
                            f'.{i_layer}', f'.{ls_id}.{layers2lslayers(i_layer)}')] = infoxlm_state_dict[key]
                elif 'fc2' in key:
                    ...
                    if i_layer % 2 == 0:
                        state_dict[
                            key.replace('decoder.sentence_encoder.layers', ls_name).replace('fc2', 'fc4').replace(
                                f'.{i_layer}', f'.{ls_id}.{layers2lslayers(i_layer)}')] = infoxlm_state_dict[key]
                    else:
                        state_dict[key.replace('decoder.sentence_encoder.layers', ls_name).replace(
                            f'.{i_layer}', f'.{ls_id}.{layers2lslayers(i_layer)}')] = infoxlm_state_dict[key]
                    ...
                    if i_layer % 2 == 1:
                        state_dict[
                            key.replace('decoder.sentence_encoder.layers', ls_name).replace(
                                'self_attn_layer_norm', 'encoder_attn_layer_norm').replace(
                                f'.{i_layer}', f'.{ls_id}.{layers2lslayers(i_layer)}')] = infoxlm_state_dict[key]
                    else:
                        state_dict[key.replace('decoder.sentence_encoder.layers', ls_name).replace(
...
class XLMTLSModel(TransformerFromPretrainedInfoXLMModel):
    ...
    def forward(
        ...
        encoder_out = self.encoder(
            src_tokens, src_lengths=src_lengths, return_all_hiddens=return_all_hiddens, src_lang_id=src_lang_id,
...
class XLMTLSDecoder(TransformerDecoder):
    def __init__(self, args, dictionary, embed_tokens, no_encoder_attn=False):
        ...
        if self.decoder_ls_depth_strategy == "static-same":
            ...
            self.decoder_ls_layers_list = [int(layer_num) for layer_num in
                                           args.decoder_ls_layers_list.split(',')] if hasattr(args,
                                                                                              "decoder_ls_layers_list") else [
                                                                                                                                 0] * len(
                self.langs)
            assert len(self.decoder_ls_layers_list) == len(self.langs)
            for i in range(len(self.langs)):
        ...
        self.softmax_temperature = args.softmax_temperature if hasattr(args, "softmax_temperature") else math.sqrt(
        ...
        if self.decoder_group_type == "sparse":
            ...
            exit()
        self.language_specific_lr = [float(w) for w in args.language_specific_lr.split(",")] if hasattr(args,
    ...
    def extract_features(
        ...
        if self.decoder_ls_depth_strategy == "static-same":
            x = torch.stack([inner_states[i] for i in
                             range(self.decoder_group_layers - 1, len(inner_states), self.decoder_group_layers)], dim=0)
        ...
        if self.language_specific_lr is not None:
            loss_weight = self.language_specific_lr[tgt_lang_id] * loss_weight
