...
class MinimizeModel(TransformerFromPretrainedInfoXLMModel):
    ...
    def forward(
        ...
        pt_encoder_out = self.encoder(
            extra["prepend_target"], src_lengths=src_lengths + 1, return_all_hiddens=return_all_hiddens,
            src_lang_id=src_lang_id, tgt_lang_id=tgt_lang_id
        )
        if "alignments" in extra.keys():
            if self.langs[src_lang_id] == self.pivot_lang:
                with torch.no_grad():
                    encoder_src_states = encoder_out["encoder_states"][-1].view(
                        encoder_out["encoder_states"][-1].size(0) * encoder_out["encoder_states"][-1].size(1),
                        encoder_out["encoder_states"][-1].size(2))
                    encoder_src_states = encoder_src_states.index_select(dim=0, index=extra["alignments"][:, 0])
                encoder_tgt_states = pt_encoder_out["encoder_states"][-1].view(
                    pt_encoder_out["encoder_states"][-1].size(0) * pt_encoder_out["encoder_states"][-1].size(1),
                    pt_encoder_out["encoder_states"][-1].size(2))
            ...
            if self.langs[src_lang_id] == self.pivot_lang:
                with torch.no_grad():
                    encoder_src_states = self.average_sequence(encoder_out['encoder_out'][0].transpose(0, 1),
                                                               encoder_out['encoder_padding_mask'][0])
                encoder_tgt_states = self.average_sequence(pt_encoder_out['encoder_out'][0].transpose(0, 1),
                                                           pt_encoder_out['encoder_padding_mask'][0])
            elif self.langs[tgt_lang_id] == self.pivot_lang:
                encoder_src_states = self.average_sequence(encoder_out['encoder_out'][0].transpose(0, 1),
                                                           encoder_out['encoder_padding_mask'][0])
                with torch.no_grad():
