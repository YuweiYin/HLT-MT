...
def CopyDecoderBottom2Top(state_dict, decoder_group_layers, decoder_group_number):
    def get_layer(_i, _j):
        return _i + _j * decoder_group_layers
...
def CopyEncoderBottom2Top(state_dict, encoder_group_layers, encoder_group_number):
    def get_layer(_i, _j):
        return _i + _j * encoder_group_layers
...
class GformerModel(TransformerFromPretrainedInfoXLMModel):
    ...
    def __init__(self, args, dictionary, embed_tokens):
        ...
        if self.random_freeze_subnet_parameter is not None:
            self.start_freeze_epoch, self.freeze_warmup_epoch, self.freeze_max_prob = \
                self.random_freeze_subnet_parameter.split(",")
...
class GformerEncoder(TransformerEncoder):
    def __init__(self, args, dictionary, embed_tokens):
        ...
        if hasattr(args, "pretrained_infoxlm_checkpoint") and os.path.exists(args.pretrained_infoxlm_checkpoint):
            ...
            if args.encoder_layers > args.encoder_group_layers and \
                    args.encoder_layers % args.encoder_group_layers == 0 and args.recurrent_init:
                infoxlm_loaded_state_dict = CopyEncoderBottom2Top(
                    infoxlm_loaded_state_dict, args.encoder_group_layers,
        ...
        self.encoder_group_type = getattr(args, "encoder_group_type", "sparse")
...
class GformerDecoder(TransformerDecoder):
    def __init__(self, args, dictionary, embed_tokens, no_encoder_attn=False):
        ...
        if hasattr(args, "pretrained_infoxlm_checkpoint") and os.path.exists(args.pretrained_infoxlm_checkpoint):
            if args.variant == 'addffn':
                ...
                if args.decoder_layers > args.decoder_group_layers and \
                        args.decoder_layers % args.decoder_group_layers == 0 and args.recurrent_init:
                    infoxlm_loaded_state_dict = CopyDecoderBottom2Top(
                        infoxlm_loaded_state_dict, args.decoder_group_layers,
        ...
        assert args.decoder_group_layers <= args.decoder_layers, \
            "Please ensure args.decoder_group_layers <= args.decoder_layers !"
        ...
        self.language_specific_lr = [float(w) for w in args.language_specific_lr.split(",")] if hasattr(
            args, "language_specific_lr") else None
    ...
    def extract_features(
        ...
        if self.language_specific_lr is not None and self.training:
