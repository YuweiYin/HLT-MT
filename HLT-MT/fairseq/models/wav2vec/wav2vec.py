...
class Wav2VecConfig(FairseqDataclass):
    ...
    conv_feature_layers: str = field(
        default="[(512, 10, 5), (512, 8, 4), (512, 4, 2), (512, 4, 2), (512, 4, 2), (512, 1, 1), (512, 1, 1), (512, "
                "1, 1)]",
        metadata={
    ...
    conv_aggregator_layers: str = field(
        default="[(512, 2, 1), (512, 3, 1), (512, 4, 1), (512, 5, 1), (512, 6, 1), (512, 7, 1), (512, 8, 1), (512, 9, "
                "1), (512, 10, 1), (512, 11, 1), (512, 12, 1), (512, 13, 1)]",
        metadata={
    ...
    vq_temp: Tuple[float, float, float] = field(
        ...
        metadata={
            "help": "temperature for latent variable sampling with gumbel softmax. should be a tuple of 3 values ("
                    "start, end, decay) "
        },
    )
...
class ConvFeatureExtractionModel(nn.Module):
    def __init__(
        ...
        def block(n_in, n_out, _k, stride):
            return nn.Sequential(
                nn.Conv1d(n_in, n_out, _k, stride=stride, bias=False),
...
class ConvAggregator(nn.Module):
    def __init__(
        ...
        def block(n_in, n_out, _k, stride):
            ...
            return nn.Sequential(
                pad,
                nn.Conv1d(n_in, n_out, _k, stride=stride, bias=conv_bias),
                nn.Dropout(p=dropout),
    ...
    def sample_negatives(self, y):
        ...
        with torch.no_grad():
            if self.n_negatives > 0:
                tszs = (
                    buffered_arange(tsz).unsqueeze(-1).expand(-1, self.n_negatives).flatten()
                )
            ...
            if self.cross_sample_negatives > 0:
                tszs = (
                    buffered_arange(tsz).unsqueeze(-1).expand(-1, self.cross_sample_negatives).flatten()
                )
