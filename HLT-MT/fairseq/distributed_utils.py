...
def infer_init_method(cfg: DistributedTrainingConfig, force_distributed=False):
    ...
    if all(
        ...
        key in os.environ
        for key in ["PHILLY_RUNTIME_CONFIG", "OMPI_COMM_WORLD_SIZE", "OMPI_COMM_WORLD_RANK",
...
def all_reduce_dict(data: Mapping[str, Any], device, group) -> Dict[str, Any]:
    ...
    def _all_reduce_dict(data: OrderedDict):
        if len(data) == 0:
            return data
        buf = torch.cat([t_v.view(-1) for t_v in data.values()]).to(device=device)
        all_reduce(buf, group=group)
        split_buf = torch.split(buf, [t_v.numel() for t_v in data.values()])
        reduced_data = [t_v.view_as(orig) for t_v, orig in zip(split_buf, data.values())]
