...
class SequenceGenerator(nn.Module):
    ...
    def _generate(
        ...
        tokens = (
            torch.zeros(bsz * beam_size, max_len + 2).to(src_tokens).long().fill_(self.pad)
        )  # +2 for eos and pad
    ...
    def transpose_list(self, li: List[List[int]]):
        # GeneratorExp aren't supported in TS so ignoring the lint
        min_len = min([len(x) for x in li])  # noqa
        l2 = [[row[i] for row in li] for i in range(min_len)]
        return l2
        ...
        for i, model in enumerate(self.models):
            ...
            if src_lang_id is not None and tgt_lang_id is not None:
                if self.has_incremental_states():
                    ...
                    decoder_out = model.decoder.forward(tokens, encoder_out=encoder_out, src_lang_id=src_lang_id,
                                                        tgt_lang_id=tgt_lang_id)
            else:
                # decode each model
                if self.has_incremental_states():
...
class SequenceGeneratorWithAlignment(SequenceGenerator):
    ...
    def _prepare_batch_for_alignment(self, sample, hypothesis):
        ...
        bsz = src_tokens.shape[0]
        src_tokens = (
            src_tokens[:, None, :].expand(-1, self.beam_size, -1).contiguous().view(bsz * self.beam_size, -1)
        )
        src_lengths = sample["net_input"]["src_lengths"]
        src_lengths = (
            src_lengths[:, None].expand(-1, self.beam_size).contiguous().view(bsz * self.beam_size)
        )
