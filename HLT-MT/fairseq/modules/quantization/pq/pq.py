...
class PQ(EM):
    ...
    def _reshape(self, W):
        """
        Reshapes the matrix W as explained in step (1).
        """
        ...
        if len(W.size()) == 2:
            ...
            return (
                W.reshape(self.out_features, -1, self.block_size).permute(2, 1, 0).flatten(1, 2)
            )
        ...
        # convolutional: we reshape along the spatial dimension
            ...
            return (
                W.reshape(self.out_channels, -1, self.block_size).permute(2, 1, 0).flatten(1, 2)
            )
        # not implemented
        else:
