...
cat "${input_file}" | python scripts/spm_encode.py --model "${spm_model}" | python scripts/multilingual/truncate.py |
  CUDA_VISIBLE_DEVICES=${cuda_device} python fairseq_cli/interactive.py "${data_bin}" \
    --path "${model_file}" \
